...
async fn provider_client_dispatches_xai_requests_from_env() {
    ...
    let server = spawn_server(
        ...
        vec![http_response(
            "200 OK",
            "application/json",
            "{\"id\":\"chatcmpl_provider\",\"model\":\"deepseek-v4-pro\",\"choices\":[{\"message\":{\"role\":\"assistant\",\"content\":\"Through provider client\",\"tool_calls\":[]},\"finish_reason\":\"stop\"}],\"usage\":{\"prompt_tokens\":9,\"completion_tokens\":4}}",
        )],
    )
    .await;
    ...
    let client =
        ProviderClient::from_model("deepseek-v4-pro").expect("deepseek provider client should be constructed");
