...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".taurus.json"),
            |parent| parent.join(".taurus.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".taurus.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".taurus").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".taurus").join("settings.local.json"),
            },
        ]
    }
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("TAURUS_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".taurus")))
        .unwrap_or_else(|| PathBuf::from(".taurus"))
}
...
fn read_optional_json_object(path: &Path) -> Result<Option<ParsedConfigFile>, ConfigError> {
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".taurus.json");
...
mod tests {
    ...
    fn rejects_non_object_settings_files() {
        ...
        let home = root.join("home").join(".taurus");
    ...
    fn loads_and_merges_claude_code_config_files_by_precedence() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".taurus");
        fs::create_dir_all(cwd.join(".taurus")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".taurus.json"),
        ...
        fs::write(
            cwd.join(".taurus.json"),
            r#"{"model":"project-compat","env":{"B":"2"}}"#,
        )
        .expect("write project compat config");
        fs::write(
            cwd.join(".taurus").join("settings.json"),
        ...
        fs::write(
            cwd.join(".taurus").join("settings.local.json"),
    ...
    fn rejects_invalid_hook_entries_before_merge() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".taurus");
        let project_settings = cwd.join(".taurus").join("settings.json");
        fs::create_dir_all(cwd.join(".taurus")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");
