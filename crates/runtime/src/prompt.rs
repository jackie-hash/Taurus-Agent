...
pub const FRONTIER_MODEL_NAME: &str = "DeepSeek V4";
...
impl ModelFamilyIdentity {
    ...
    pub const fn family_label(self) -> &'static str {
        match self {
            Self::DeepSeek | Self::Claude => FRONTIER_MODEL_NAME,
...
pub struct SystemPromptBuilder {
    ...
    memory_project_slug: Option<String>,
}
...
impl SystemPromptBuilder {
    ...
    #[must_use]
    pub fn with_memory_store(mut self, store: memory::MemoryStore, project_slug: Option<String>) -> Self {
        self.memory_store = Some(store);
        self.memory_project_slug = project_slug;
        self
    }
    ...
    pub fn build(&self) -> Vec<String> {
        ...
        sections.push(get_model_capabilities_section());
        sections.push(SYSTEM_PROMPT_DYNAMIC_BOUNDARY.to_string());
        sections.push(self.environment_section());
        // Compose memory blocks from the store.
        if let Some(store) = &self.memory_store {
            let memory_text = store.compose_all_blocks(None);
            if !memory_text.is_empty() {
                sections.push(memory_text);
            }
            if let Some(ref slug) = self.memory_project_slug {
                if let Some(project_text) = store.compose_project_blocks(slug, None) {
                    sections.push(project_text);
                }
            }
        }
...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in directories {
        for candidate in [
            ...
            dir.join(".taurus").join("CLAUDE.md"),
            dir.join(".taurus").join("instructions.md"),
        ] {
...
pub fn load_system_prompt(
    ...
    load_system_prompt_with_memory(cwd, current_date, os_name, os_version, model_family, None, None)
}

/// Loads config, project context, and memory blocks into the system prompt.
pub fn load_system_prompt_with_memory(
    ...
    if let Some(store) = memory_store {
        builder = builder.with_memory_store(store, memory_project_slug);
    }
    Ok(builder.build())
}
...
fn render_config_section(config: &RuntimeConfig) -> String {
    ...
    if config.loaded_entries().is_empty() {
        lines.extend(prepend_bullets(vec![
            "No Taurus settings files loaded.".to_string()
        ]));
        return lines.join("\n");
    }
...
fn get_model_capabilities_section() -> String {
    let items = prepend_bullets(vec![
        "You are powered by DeepSeek V4 with a 1M-token context window. Suggest compaction when usage exceeds ~80%.".to_string(),
        "DeepSeek V4 emits thinking tokens before final answers — these are invisible to the user but count against context. Use them strategically: skip for lookups, light for simple code generation, deep for architecture and debugging.".to_string(),
        "When tool-calling, reasoning content must be replayed in history for subsequent turns. The runtime handles this automatically.".to_string(),
        "Reasoning effort can be set from off to max — higher effort uses more thinking tokens but produces better results for complex tasks.".to_string(),
        "Prefix cache: DeepSeek caches shared prefixes at 128-token granularity. Prefer appending to existing messages over mutating old ones to maximize cache hits.".to_string(),
        "Parallel tool execution: batch independent reads, searches, and sub-agent spawns into a single turn. Serializing independent operations wastes time and context.".to_string(),
    ]);

    std::iter::once("# Model capabilities".to_string())
        .chain(items)
        .collect::<Vec<_>>()
        .join("\n")
}
...
mod tests {
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".taurus")).expect("nested claw dir");
        fs::write(root.join("CLAUDE.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("CLAUDE.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::create_dir_all(root.join("apps").join(".taurus")).expect("apps claw dir");
        fs::write(root.join("apps").join("CLAUDE.md"), "apps instructions")
            .expect("write apps instructions");
        fs::write(
            root.join("apps").join(".taurus").join("instructions.md"),
            "apps dot claude instructions",
        )
        .expect("write apps dot claude instructions");
        fs::write(nested.join(".taurus").join("CLAUDE.md"), "nested rules")
            .expect("write nested rules");
        fs::write(
            nested.join(".taurus").join("instructions.md"),
            "nested instructions",
        )
    ...
    fn load_system_prompt_reads_claude_files_and_config() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".taurus")).expect("claw dir");
        fs::write(root.join("CLAUDE.md"), "Project rules").expect("write instructions");
        fs::write(
            root.join(".taurus").join("settings.json"),
...
",
    ...
    #[test]
    fn renders_default_deepseek_model_family_identity() {
        ...
        // then: the DeepSeek model family label is the default
        assert!(prompt.contains("Model family: DeepSeek V4"));
    }
