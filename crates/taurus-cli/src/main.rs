...
use runtime::{
    ...
    load_system_prompt, load_system_prompt_with_memory, pricing_for_model,
...
enum ModelSource {
    ...
    Flag,
    /// DEEPSEEK_MODEL environment variable (when no flag was passed).
    Env,
...
impl ModelProvenance {
    ...
    fn from_env_or_config_or_default(cli_model: &str) -> Self {
        ...
        if let Some(env_model) = env::var("DEEPSEEK_MODEL")
            .ok()
        ...
        other => {
            if rest.len() == 1 && looks_like_subcommand_typo(other) {
                if let Some(suggestions) = suggest_similar_subcommand(other) {
                    ...
                    message.push_str(
                        "\nRun `taurus --help` for the full list. If you meant to send a prompt literally, use `taurus prompt <text>`.",
                    );
                    return Err(message);
                }
            }
...
fn removed_auth_surface_error(command_name: &str) -> String {
    format!(
        "`taurus {command_name}` has been removed. Set DEEPSEEK_API_KEY or DEEPSEEK_AUTH_TOKEN instead."
    )
}
...
fn parse_direct_slash_cli_action(
    ...
    match SlashCommand::parse(&raw) {
        ...
        Ok(Some(command)) => Err({
            let _ = command;
            format!(
                "slash command {command_name} is interactive-only. Start `taurus` and run it there, or use `taurus --resume SESSION.jsonl {command_name}` / `taurus --resume {latest} {command_name}` when the command is marked [resume] in /help.",
                command_name = rest[0],
...
fn format_unknown_direct_slash_command(name: &str) -> String {
    ...
    message.push_str("\nRun `taurus --help` for CLI usage, or start `taurus` and use /help.");
    message
}
...
/// Accepts: known aliases (pro, flash) or model name pattern.
...
fn validate_model_syntax(model: &str) -> Result<(), String> {
    ...
    if parts.len() != 2 || parts[0].is_empty() || parts[1].is_empty() {
        ...
        let mut err_msg = format!(
            "invalid model syntax: '{}'. Expected model name (e.g., deepseek-v4-pro) or known alias (pro, flash)",
            trimmed
        );
...
fn run_worker_state(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !state_path.exists() {
        ...
        return Err(format!(
            "no worker state file found at {path}\n  Hint: worker state is written by the interactive REPL or a non-interactive prompt.\n  Run:   taurus               # start the REPL (writes state on first turn)\n  Or:    taurus prompt <text> # run one non-interactive turn\n  Then rerun: taurus state [--output-format json]",
...
#[allow(clippy::too_many_lines)]
fn dotenv_fallback(key: &str) -> Option<String> {
...
fn check_auth_health() -> DiagnosticCheck {
    let api_key_present = env::var("DEEPSEEK_API_KEY")
        ...
        .or_else(|| dotenv_fallback("DEEPSEEK_API_KEY"))
        .is_some();
    let auth_token_present = env::var("DEEPSEEK_AUTH_TOKEN")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .or_else(|| dotenv_fallback("DEEPSEEK_AUTH_TOKEN"))
        .is_some();
    ...
    match load_oauth_credentials() {
        ...
        .with_details(vec![
            ...
            "Suggested action  set DEEPSEEK_API_KEY or DEEPSEEK_AUTH_TOKEN; `claw login` is removed"
                .to_string(),
        ])
    ...
    match command {
        ...
        SlashCommand::Skills { args } => {
            if let SkillSlashDispatch::Invoke(_) = classify_skills_slash_command(args.as_deref()) {
                return Err(
                    "resumed /skills invocations are interactive-only; start `taurus` and run `/skills <skill>` in the REPL".into(),
                );
            }
            let cwd = env::current_dir()?;
            Ok(ResumeCommandOutcome {
        ...
        SlashCommand::Plugins { action, target } => {
            ...
            match action.as_deref() {
                ...
                | Some("update") => {
                    return Err(
                        "resumed /plugins mutations are interactive-only; start `taurus` and run `/plugins` in the REPL".into(),
                    );
                }
                _ => {}
            }
...
fn enforce_broad_cwd_policy(
    ...
    if is_interactive {
        ...
        eprintln!(
            "Warning: taurus is running from a very broad directory ({}).\n\
             The agent can read and search everything under this path.\n\
             Consider running from inside your project: cd /path/to/project && taurus",
            cwd.display()
        );
        ...
        let message = format!(
            "taurus is running from a very broad directory ({}). \
             The agent can read and search everything under this path. \
             Use --allow-broad-cwd to proceed anyway, \
             or run from inside your project: cd /path/to/project && taurus",
            cwd.display()
        );
        match output_format {
            CliOutputFormat::Json => {
...
struct BuiltRuntime {
    runtime: Option<ConversationRuntime<TaurusRuntimeClient, CliToolExecutor>>,
    plugin_registry: PluginRegistry,
    plugins_active: bool,
    mcp_state: Option<Arc<Mutex<RuntimeMcpState>>>,
    mcp_active: bool,
...
impl BuiltRuntime {
    fn new(
        runtime: ConversationRuntime<TaurusRuntimeClient, CliToolExecutor>,
        plugin_registry: PluginRegistry,
        mcp_state: Option<Arc<Mutex<RuntimeMcpState>>>,
    ) -> Self {
        Self {
...
impl Deref for BuiltRuntime {
    type Target = ConversationRuntime<TaurusRuntimeClient, CliToolExecutor>;
...
fn render_help_topic(topic: LocalHelpTopic) -> String {
  ...
  Usage            taurus state [--output-format <format>]
  ...
  Formats          text (default), json
  Produces state   `taurus` (interactive REPL) or `taurus prompt <text>` (one non-interactive turn)
  Observes state   `taurus state` reads; tauruship/CI may poll this file without HTTP
  Exit codes       0 if state file exists and parses; 1 with actionable hint otherwise
  Related          taurus status · ROADMAP #139 (this worker-concept contract)"
            .to_string(),
...
fn render_export_help_json() -> serde_json::Value {
    json!({
        ...
        "topic": "export",
        "command": "export",
        "usage": "taurus export [--session <id|latest>] [--output <path>] [--output-format <format>]",
        "purpose": "serialize a managed session to JSON for review, transfer, or archival",
        "defaults": {
        ...
        "related": ["/session list", "taurus --resume latest"]
    })
}
...
fn print_acp_status(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match output_format {
        CliOutputFormat::Text => {
            println!(
                "ACP / Zed\n  Status           discoverability only\n  Launch           `taurus acp serve` / `taurus --acp` / `taurus -acp` report status only; no editor daemon is available yet\n  Today            use `taurus prompt`, the REPL, or `taurus doctor` for local verification\n  Tracking         ROADMAP #76\n  Message          {message}"
            );
        }
        CliOutputFormat::Json => {
            println!(
...
fn build_system_prompt(model: &str) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let project_slug = project_slug_from_cwd(&cwd);
    let memory_store = memory::MemoryStore::default_store();
    let _ = memory_store.init();
    Ok(load_system_prompt_with_memory(
        cwd,
        DEFAULT_DATE,
        env::consts::OS,
        "unknown",
...
fn project_slug_from_cwd(cwd: &Path) -> String {
    cwd.file_name()
        .and_then(|n| n.to_str())
        .unwrap_or("default")
        .to_string()
...
/// DeepSeek-native runtime client. Holds a [`ProviderClient`] which wraps
/// an OpenAI-compatible HTTP client configured for DeepSeek's API.
struct TaurusRuntimeClient {
    runtime: tokio::runtime::Runtime,
...
impl ApiClient for TaurusRuntimeClient {
    #[allow(clippy::too_many_lines)]
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    ...
    writeln!(
        out,
        "  taurus [--model MODEL] [--allowedTools TOOL[,TOOL...]]"
    )?;
    writeln!(out, "      Start the interactive REPL")?;
    writeln!(
        out,
        "  taurus [--model MODEL] [--output-format text|json] prompt TEXT"
    )?;
    writeln!(out, "      Send one prompt and exit")?;
    writeln!(
        out,
        "  taurus [--model MODEL] [--output-format text|json] TEXT"
    )?;
    writeln!(out, "      Shorthand non-interactive prompt mode")?;
    writeln!(
        out,
        "  taurus --resume [SESSION.jsonl|session-id|latest] [/status] [/compact] [...]"
    )?;
    writeln!(
        out,
        "      Inspect or maintain a saved session without entering the REPL"
    )?;
    writeln!(out, "  taurus help")?;
    writeln!(out, "      Alias for --help")?;
    writeln!(out, "  taurus version")?;
    writeln!(out, "      Alias for --version")?;
    writeln!(out, "  taurus status")?;
    writeln!(
        out,
        "      Show the current local workspace status snapshot"
    )?;
    writeln!(out, "  taurus sandbox")?;
    writeln!(out, "      Show the current sandbox isolation snapshot")?;
    writeln!(out, "  taurus doctor")?;
    writeln!(
        out,
        "      Diagnose local auth, config, workspace, and sandbox health"
    )?;
    writeln!(out, "  taurus acp [serve]")?;
    writeln!(
        out,
    ...
    writeln!(out, "  taurus dump-manifests [--manifests-dir PATH]")?;
    writeln!(out, "  taurus bootstrap-plan")?;
    writeln!(out, "  taurus agents")?;
    writeln!(out, "  taurus mcp")?;
    writeln!(out, "  taurus skills")?;
    writeln!(out, "  taurus system-prompt [--cwd PATH] [--date YYYY-MM-DD]")?;
    writeln!(out, "  taurus init")?;
    writeln!(
        out,
        "  taurus export [PATH] [--session SESSION] [--output PATH]"
    )?;
    writeln!(
        out,
    ...
    writeln!(out, "  taurus --model deepseek-v4-pro \"summarize this repo\"")?;
    writeln!(
        out,
        "  taurus --output-format json prompt \"explain src/main.rs\""
    )?;
    writeln!(out, "  taurus --compact \"summarize Cargo.toml\" | wc -l")?;
    writeln!(
        out,
        "  taurus --allowedTools read,glob \"summarize Cargo.toml\""
    )?;
    writeln!(out, "  taurus --resume {LATEST_SESSION_REFERENCE}")?;
    writeln!(
        out,
        "  taurus --resume {LATEST_SESSION_REFERENCE} /status /diff /export notes.txt"
    )?;
    writeln!(out, "  taurus agents")?;
    writeln!(out, "  taurus mcp show my-server")?;
    writeln!(out, "  taurus /skills")?;
    writeln!(out, "  taurus doctor")?;
    ...
    writeln!(out, "  taurus init")?;
    writeln!(out, "  taurus export")?;
    writeln!(out, "  taurus export conversation.md")?;
    Ok(())
}
...
mod tests {
    ...
    fn context_window_preflight_errors_render_recovery_steps() {
        ...
        assert!(
            rendered.contains("Model            deepseek-v4-flash"),
            "{rendered}"
        );
        assert!(
    ...
    fn user_defined_aliases_resolve_before_provider_dispatch() {
        ...
        std::fs::write(
            cwd.join(".claw").join("settings.json"),
            r#"{"aliases":{"fast":"deepseek-v4-flash","smart":"pro","cheap":"grok-3-mini"}}"#,
        )
        .expect("project config should write");
        ...
        let builtin = with_current_dir(&cwd, || resolve_model_alias_with_config("flash"));
    ...
    fn removed_login_and_logout_subcommands_error_helpfully() {
        ...
        assert!(logout.contains("DEEPSEEK_AUTH_TOKEN"));
        assert_eq!(
        ...
        .expect("--model sonnet status should parse")
        {
            CliAction::Status {
                ...
                assert_eq!(model, "deepseek-v4-flash", "sonnet alias should resolve");
                assert_eq!(
                    model_flag_raw.as_deref(),
                    Some("flash"),
    ...
    fn export_help_json_is_bounded_and_parseable_384() {
        ...
        assert_eq!(
            value["usage"],
            "taurus export [--session <id|latest>] [--output <path>] [--output-format <format>]"
        );
...
"#,
    ...
    fn state_error_surfaces_actionable_worker_commands_139() {
        ...
        assert!(
            message.contains("taurus prompt"),
            "error should name `taurus prompt <text>` as a producer: {message}"
        );
        assert!(
        ...
        assert!(
            state_help.contains("taurus prompt"),
            "state help must name `taurus prompt <text>` as a producer: {state_help}"
        );
    }
    ...
    fn multi_word_prompt_still_uses_shorthand_prompt_mode() {
        let _guard = env_lock();
        std::env::remove_var("RUSTY_CLAUDE_PERMISSION_MODE");
        // Input is ["--model", "pro", "please", "debug", "this"] so the joined
    ...
    fn parses_direct_agents_mcp_and_skills_slash_commands() {
        ...
        assert!(error.contains("taurus --resume SESSION.jsonl /status"));
    }
    ...
    fn rejects_unknown_options_with_helpful_guidance() {
        ...
        assert!(error.contains("taurus --help"));
    }
    ...
    fn completion_candidates_include_workflow_shortcuts_and_dynamic_sessions() {
        ...
        assert!(completions.contains(&"/model deepseek-v4-flash".to_string()));
    ...
    fn format_connected_line_renders_deepseek_provider_for_deepseek_model() {
        ...
        assert_eq!(line, "Connected: deepseek-v4-flash via deepseek");
    }

    #[test]
    fn format_connected_line_renders_deepseek_provider() {
        let model = "grok-3";

        let line = format_connected_line(model);

        assert_eq!(line, "Connected: grok-3 via deepseek");
    }
    ...
    fn resolve_repl_model_falls_back_to_anthropic_model_env_when_default() {
        ...
        std::env::set_var("DEEPSEEK_MODEL", "flash");
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("taurus help"));
        assert!(help.contains("taurus version"));
        assert!(help.contains("taurus status"));
        assert!(help.contains("taurus sandbox"));
        assert!(help.contains("taurus init"));
        assert!(help.contains("taurus acp [serve]"));
        assert!(help.contains("taurus agents"));
        assert!(help.contains("taurus mcp"));
        assert!(help.contains("taurus skills"));
        assert!(help.contains("taurus /skills"));
        assert!(help.contains("ultraworkers/claw-code"));
        assert!(help.contains("cargo install claw-code"));
        assert!(!help.contains("taurus login"));
        assert!(!help.contains("taurus logout"));
    }
...
UU conflicted.rs",
    ...
    fn help_mentions_jsonl_resume_examples() {
        ...
        print_help_to(&mut help).expect("help should render");
        let help = String::from_utf8(help).expect("help should be utf8");
        assert!(help.contains("taurus --resume [SESSION.jsonl|session-id|latest]"));
        assert!(help.contains("Use `latest` with --resume, /resume, or /session switch"));
        assert!(help.contains("taurus --resume latest"));
        assert!(help.contains("taurus --resume latest /status /diff /export notes.txt"));
    }
